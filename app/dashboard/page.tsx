...
function Sidebar({
    ...
    return (
        <>
            ...
            <aside
                className={`
          sidebar fixed lg:sticky top-0 left-0 h-screen z-50 flex flex-col
                ...
                <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto overflow-x-hidden">
                    {navItems.map((item) => (
                        <button
                            key={item.label}
                            onClick={onClose}
                            className={`nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${item.active
      ...
      `}</style>
                ...
                <header className="sticky top-0 z-20 bg-white header-shadow flex items-center justify-between px-4 md:px-8 py-3 md:py-4">
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="collapse-btn lg:hidden w-9 h-9 rounded-lg flex items-center justify-center"
                        >
                        ...
                        <button
                            ...
                            className="collapse-btn hidden lg:flex w-8 h-8 rounded-lg items-center justify-center"
                        >
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs hidden sm:block">Bem-vindo de volta, {user?.displayName?.split(" ")[0] ?? "Ricardo"} 👋</p>
                        </div>
                    </div>
                        ...
                        <div className="chart-card xl:col-span-2 p-4 md:p-6">
                            ...
                            <svg viewBox="0 0 600 200" className="w-full" style={{ height: 160 }}>
                                ...
                                {months.map((m, i) => {
                                    ...
                                    const revH = (rev / maxBar) * 160; const expH = (exp / maxBar) * 160;
                                    return (
                                        <g key={m}>
                                            <rect className="bar-rev bar-rev-anim" x={x} y={170 - revH} width={bw} height={revH} rx="3" style={{ animationDelay: `${i * 60}ms` }} />
                                            <rect className="bar-exp bar-exp-anim" x={x + bw + 2} y={170 - expH} width={bw} height={expH} rx="3" style={{ animationDelay: `${i * 60 + 30}ms` }} />
                            ...
                            <div className="flex items-center justify-between mb-4 md:mb-5">
                                ...
                                <div className="flex items-center gap-2">
                                    <button className="flex items-center gap-1 bg-slate-50 hover:bg-blue-50 text-slate-500 hover:text-blue-600 text-xs font-medium px-2.5 py-1.5 rounded-lg transition-colors">
                                        <Filter size={11} /> <span className="hidden sm:inline">Filtrar</span>
                                    </button>
                                    <button className="flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-colors">
                                        Ver todas
                                    </button>
                                </div>
                            </div>
                                ...
                                <svg width="130" height="130" viewBox="0 0 140 140">
                                    {(() => {
                                        ...
                                        return costCenters.map((cc) => {
                                            ...
                                            const el = (
                                                <circle key={cc.name} cx={70} cy={70} r={52} fill="none" stroke={cc.color} strokeWidth={22}
                                                    strokeDasharray={`${dash} ${circ - dash}`} strokeDashoffset={-offset}
                                                    transform="rotate(-90 70 70)" className="donut-ring" />
                                            );
                                            offset += dash;
                                            return el;
                                        });
                        ...
                        <div className="mt-4 md:mt-5">
                            <div className="flex justify-between text-xs text-slate-400 mb-1.5">
                                ...
                                <span className="mono font-semibold text-emerald-600">
                                    R$ 1.472.890 <span className="text-slate-400 font-normal">(+18.0%)</span>
                                </span>
                            </div>
            ...
            <nav className="bottom-nav fixed bottom-0 left-0 right-0 z-30 flex lg:hidden justify-around items-center py-2 px-2">
                {[
                    ...
                    <button
                        key={item.label}
                        className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all ${item.active ? "text-blue-600" : "text-slate-400 hover:text-blue-500"
                            }`}
                    >

...
type Canal = "mercadolivre" | "shopee" | "tiktokshop" | "shein" | "manual";
...
export default function VendasPage() {
  ...
  const serie = useMemo(() => {
    const buckets: { key: string; label: string; ml: number; shopee: number; tiktokshop: number; shein: number; manual: number }[] = [];

    // "mercadolivre" → ml, "shopee" → shopee, "tiktokshop" → tiktokshop, "shein" → shein, resto (manual) → manual.
    const chKey = (c?: Canal): "ml" | "shopee" | "tiktokshop" | "shein" | "manual" =>
      c === "shopee" ? "shopee" : c === "mercadolivre" ? "ml" : c === "tiktokshop" ? "tiktokshop" : c === "shein" ? "shein" : "manual";

    const [y, m] = mesSelecionado.split("-").map(Number);
    const dias = new Date(y, m, 0).getDate();
    for (let i = 0; i < dias; i++) {
      const d = new Date(y, m - 1, i + 1);
      buckets.push({ key: ymd(d), label: String(d.getDate()), ml: 0, shopee: 0, tiktokshop: 0, shein: 0, manual: 0 });
    }
    vendas.forEach((v) => {
    ...
    const max = Math.max(1, ...buckets.map((b) => b.ml + b.shopee + b.tiktokshop + b.shein + b.manual));
    return { buckets, max };
      ...
      <main className="px-6 py-8 max-w-7xl mx-auto w-full space-y-8 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="cf-card p-5 space-y-4">
            ...
            {/* ML + TikTok Shop + venda manual (+ Shopee/Shein quando visíveis — ver SHOPEE_UI_VISIVEL/SHEIN_UI_VISIVEL). */}
            {CANAIS_QUEBRA.map((c) => {
              const d = c === "mercadolivre" ? kpis.ml : c === "shopee" ? kpis.shopee : c === "tiktokshop" ? kpis.tiktokshop : c === "shein" ? kpis.shein : kpis.manual;
          ...
          <div className="cf-card p-5 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-[11px]" style={{ color: "var(--cf-text-3)" }}>
                ...
                {SHEIN_UI_VISIVEL && (
                  <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm inline-block" style={{ background: CANAL_INFO.shein.solid }} /> Shein</span>
                )}
              </div>
            </div>
            {vendas.length === 0 ? (
              ...
              >
                {serie.buckets.map((b, i) => {
                  ...
                  const hShein = (b.shein / serie.max) * 150;
                  return (
                    <g key={b.key}>
                      ...
                      {SHEIN_UI_VISIVEL && (
                        <rect x={x} y={165 - hMl - hMan - hShp - hTts - hShein} width={bw} height={hShein} rx="2" fill={CANAL_INFO.shein.solid} />
                      )}
              ...
              <table className="sr-only">
                ...
                <thead>
                  <tr>
                    ...
                    {SHEIN_UI_VISIVEL && <th scope="col">{CANAL_INFO.shein.label}</th>}
                  </tr>
                </thead>
                <tbody>
                  {serie.buckets.map((b) => (
                    <tr key={b.key}>
                      ...
                      {SHEIN_UI_VISIVEL && <td>{toBRL(b.shein, locale)}</td>}
                    </tr>
                  ))}
                </tbody>
              </table>

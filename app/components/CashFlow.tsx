...
function PrevisaoModal({ open, value, onClose, onSave }: {
  ...
  useEffect(() => {
    if (open) setRaw(value > 0 ? value.toFixed(2).replace(".", ",") : "");
  }, [open, value]);
  ...
  return (
    ...
    >
      <div
        className="cf-card p-6 w-full max-w-xs"
        style={{ animation: "mIn .2s cubic-bezier(.22,.68,0,1.2)" }}
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="font-bold text-sm" style={{ color: "var(--cf-text)" }}>Definir previsão</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--cf-text2)" }}>Valor orçado para o período</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-1.5 mb-4">
          <label className="text-xs font-bold uppercase tracking-wide" style={{ color: "var(--cf-text2)" }}>
            Valor (R$)
          </label>
          <input
            inputMode="decimal"
...
function Modal({ open, editing, uid, onClose, onSave }: {
  ...
  const missing = [!descOk && "descrição", !catOk && "categoria", !amtOk && "valor", !dateOk && "data"]
    .filter(Boolean).join(", ");
  ...
  const uploadNf = async (): Promise<{ url: string; name: string }> => {
    ...
    const timeout = new Promise<never>((_, rej) =>
      setTimeout(() => rej(new Error("Timeout: Firebase Storage pode não estar ativado.")), 15000)
    );
    const upload = async () => {
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 32px 80px rgba(10,22,40,0.3)",
          animation: "mIn .25s cubic-bezier(.22,.68,0,1.2)",
        }}
      >
        ...
        >
          <div>
            <p className="font-extrabold text-base" style={{ color: "var(--cf-text)" }}>
              {editing ? "Editar transação" : "Nova transação"}
            </p>
            <p className="text-xs" style={{ color: "var(--cf-text2)" }}>
              Preencha os campos ou anexe uma NF
            </p>
          </div>
          <button
        ...
        <div className="px-5 pt-4 pb-6 space-y-4 overflow-y-auto" style={{ maxHeight: "82vh" }}>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <span className="font-normal normal-case" style={{ color: "var(--cf-text3)" }}>
                (opcional)
              </span>
            </label>
            {hasNf ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: "#60a5fa" }}>Anexada ✓</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  {(nfPreview || nfUrl) && (
                    <a
                      href={nfPreview || nfUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer"
                  ...
                  <button
                    onClick={() => { setNfFile(null); setNfPreview(""); setNfUrl(""); setNfName(""); }}
                    className="w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer"
                    style={{ background: "#dbeafe", color: "#60a5fa" }}
                  >
                    <X size={12} />
                  </button>
                </div>
              ...
              <button
                onClick={() => nfRef.current?.click()}
                className="w-full border-2 border-dashed rounded-xl p-4 flex items-center gap-3 transition-all cursor-pointer"
                style={{ borderColor: "var(--cf-border)", background: "transparent" }}
              >
                <div
                  className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                  style={{ background: "var(--cf-input)" }}
                >
                  <Upload size={15} style={{ color: "var(--cf-text3)" }} />
                </div>
          ...
          <div className="grid grid-cols-2 gap-2 p-1 rounded-xl" style={{ background: "var(--cf-input)" }}>
            {(["entrada", "saida"] as TxType[]).map((t) => (
              <button
                ...
                style={
                  type === t
                    ? { background: t === "entrada" ? "#10b981" : "#ef4444", color: "white" }
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl px-3 py-2.5 pr-9 text-sm outline-none cursor-pointer transition-all"
                style={{ background: "var(--cf-input)", border: "1px solid var(--cf-border)", color: "var(--cf-text)" }}
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-xl px-3 py-2.5 text-sm outline-none cursor-pointer transition-all"
                style={{ background: "var(--cf-input)", border: "1px solid var(--cf-border)", color: "var(--cf-text)" }}
              />
            </div>
          </div>
          ...
          <button
            ...
            style={
              canSave && !saving
                ? { background: type === "entrada" ? "#10b981" : "#ef4444", color: "white" }
...
function ImportModal({ open, onClose, onImport }: {
  ...
  const normalizeCategory = (raw: string, type: TxType): string => {
    const cats = type === "entrada" ? CAT.entrada : CAT.saida;
    const match = cats.find(
      (c) => c.toLowerCase().includes(raw.toLowerCase()) || raw.toLowerCase().includes(c.toLowerCase())
    );
  ...
  const analyze = async () => {
    ...
    try {
      ...
      if (!parsed.transactions?.length) {
        setErrMsg("Nenhuma transação encontrada.");
        setStep("input");
        return;
      }
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-lg sm:rounded-2xl rounded-t-3xl overflow-hidden"
        style={{
          background: "var(--cf-card)",
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-extrabold text-base leading-tight" style={{ color: "var(--cf-text)" }}>
                Importar com IA
              </p>
              <p className="text-xs" style={{ color: "var(--cf-text2)" }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {step === "input" && (
            <>
              ...
              <button
                ...
                className="w-full border-2 border-dashed rounded-xl p-5 flex flex-col items-center gap-2 transition-all cursor-pointer"
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-xs mt-1" style={{ color: "var(--cf-text2)" }}>
                  Identificando transações, valores e categorias
                </p>
              </div>
              <div className="w-48 h-1.5 rounded-full overflow-hidden" style={{ background: "var(--cf-border)" }}>
                <div className="h-full rounded-full animate-pulse" style={{ width: "60%", background: "#3b82f6" }} />
              </div>
            </div>
          )}
          ...
          {(step === "preview" || step === "saving") && (
            <>
              ...
              <button
                onClick={toggleAll}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-xl cursor-pointer text-sm font-medium transition-colors"
                style={{ border: "1px solid var(--cf-border)", color: "var(--cf-text)" }}
              >
                <div
                ...
                <span className="ml-auto text-xs" style={{ color: "var(--cf-text2)" }}>
                  {selected.size}/{preview.length}
                </span>
              </button>
              <div className="space-y-1.5">
                  ...
                  return (
                    <button
                      ...
                      style={
                        isSel
                          ? {
                            borderColor: tx.type === "entrada" ? "#6ee7b7" : "#fca5a5",
                            background: tx.type === "entrada" ? "#f0fdf4" : "#fff1f2",
                          }
                          : { borderColor: "var(--cf-border)", background: "var(--cf-input)" }
                      }
                    >
                      <div
                      ...
                      <div
                        ...
                        style={{ background: tx.type === "entrada" ? "#d1fae5" : "#fee2e2" }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs truncate" style={{ color: "var(--cf-text2)" }}>
                          {tx.category} · {labelDate(tx.date)}
                        </p>
                      </div>
                      <span
                        className="text-xs font-bold mono shrink-0"
                        style={{ color: tx.type === "entrada" ? "#059669" : "#dc2626" }}
                      >
          ...
          {step === "done" && (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              ...
              <p className="text-sm text-center" style={{ color: "var(--cf-text2)" }}>
                {selected.size} transação{selected.size !== 1 ? "ões" : ""} adicionada{selected.size !== 1 ? "s" : ""}.
              </p>
            </div>
          )}
        </div>
        ...
        >
          ...
          {step === "saving" && (
            <button
              disabled
              className="flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 cursor-not-allowed opacity-50"
              style={{ background: "var(--cf-input)", color: "var(--cf-text2)" }}
            >
...
export default function CashFlowPage() {
  ...
  async function handleImport(importedTxs: ImportedTx[]) {
    ...
    const { db } = await getFirebase();
    await Promise.all(
      importedTxs.map((tx) => addDoc(collection(db, "users", uid, "cashflow"), { ...tx, createdAt: Date.now() }))
    );
  }
  ...
  const entradas = useMemo(
    () => txs.filter((t) => t.type === "entrada").reduce((s, t) => s + t.amount, 0),
    [txs]
  );
  const saidas = useMemo(
    () => txs.filter((t) => t.type === "saida").reduce((s, t) => s + t.amount, 0),
    [txs]
  );
  ...
  const kpis = [
    {
      ...
      clickable: true,
      onClick: () => setPrevisaoOpen(true),
    },
    {
      label: "Entradas",
      val: entradas,
      Icon: ArrowUpRight,
      ibg: dark ? "#052e16" : "#dcfce7",
      color: "#22c55e",
      sub: `${txs.filter((t) => t.type === "entrada").length} lançamentos`,
    },
    {
      label: "Saídas",
      val: saidas,
      Icon: ArrowDownRight,
      ibg: dark ? "#2d0a0a" : "#fee2e2",
      color: "#ef4444",
      sub: `${txs.filter((t) => t.type === "saida").length} lançamentos`,
    },
    {
      label: "Saldo atual",
      val: saldo,
      Icon: Wallet,
      ibg: saldo >= 0 ? (dark ? "#0c1a2e" : "#dbeafe") : (dark ? "#2d0a0a" : "#fee2e2"),
      color: saldo >= 0 ? "#3b82f6" : "#ef4444",
      sub: saldo >= 0 ? "Resultado positivo" : "Resultado negativo",
    },
    {
      label: "Superávit / Déficit",
      val: superavitDeficit,
      Icon: TrendingUp,
      ibg: superavitDeficit >= 0 ? (dark ? "#052e16" : "#dcfce7") : (dark ? "#2d0a0a" : "#fee2e2"),
      color: superavitDeficit >= 0 ? "#22c55e" : "#ef4444",
      sub: previsao === 0 ? "Defina a previsão" : superavitDeficit >= 0 ? "Acima do previsto" : "Abaixo do previsto",
    },
  ];
  ...
  if (pageState === "loading") return (
    <div
      ...
      style={{ background: "var(--cf-bg)", ...(theme as any) }}
    >
  ...
  if (pageState === "error") return (
    ...
    >
      ...
      <div
        className="rounded-xl p-4 max-w-sm w-full text-left"
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "var(--cf-bg)", ...(theme as any) }}>
      <style>{`
        ...
        *, *::before, *::after { font-family: 'Sora', sans-serif; box-sizing: border-box; }
        ...
        .cf-kpi:hover { border-color: var(--cf-text3); transform: translateY(-1px); }
        ...
        /* Animations */
        @keyframes mIn { from { opacity: 0; transform: translateY(20px) scale(.98) } to { opacity: 1; transform: none } }
        ...
        /* Progress bar for saldo */
        .cf-progress { height: 3px; border-radius: 2px; background: var(--cf-border); overflow: hidden; margin-top: 6px; }
        .cf-progress-fill { height: 100%; border-radius: 2px; transition: width .6s cubic-bezier(.4,0,.2,1); }

        /* Subtle transitions */
        * { transition: background-color .18s, border-color .18s, color .12s; }
        button, a, input, select, textarea { transition: background-color .12s, border-color .12s, color .08s, opacity .12s, transform .1s !important; }
      `}</style>
      ...
      {/* Main */}
      <main className="flex-1 p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 overflow-y-auto overflow-x-hidden pb-24 lg:pb-8">

        {/* Header */}
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="font-bold text-xl leading-tight" style={{ color: "var(--cf-text)" }}>
              Fluxo de caixa
            </h1>
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button
              ...
              style={{ border: "1px solid var(--cf-border)", background: "var(--cf-card)", color: "var(--cf-text2)" }}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
          {kpis.map(({ label, val, Icon, ibg, color, sub, clickable, onClick }, idx) => {
            // barra de progresso: só para "Entradas" e "Saídas" mostra % do total movimentado
            const total = entradas + saidas;
            const pct = total > 0 ? (Math.abs(val) / total) * 100 : 0;
            const showBar = label === "Entradas" || label === "Saídas";
            const isSaldo = label === "Saldo atual";
            const isSuperavit = label === "Superávit / Déficit";
            ...
            return (
              ...
              >
                ...
                {/* Valor */}
                <div>
                  <p className="font-extrabold text-base sm:text-[17px] mono leading-tight" style={{ color }}>
                    {(isSuperavit || isSaldo) && val > 0 ? "+" : ""}
                    {toBRL(val)}
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: "var(--cf-text3)" }}>{sub}</p>
                </div>
        ...
        <button
          onClick={() => setImportOpen(true)}
          className="sm:hidden w-full cf-card p-3 flex items-center gap-3 cursor-pointer"
        >
          <div
          ...
          <div className="flex-1 text-left">
            <p className="text-sm font-bold leading-tight" style={{ color: "var(--cf-text)" }}>
              Importar extrato bancário
            </p>
            <p className="text-xs" style={{ color: "var(--cf-text2)" }}>A IA preenche tudo automaticamente</p>
          </div>
          <ArrowUpRight size={15} style={{ color: "var(--cf-text3)" }} />
        </button>

        {/* Toolbar */}
        <div className="cf-card p-3 space-y-2.5">
          <div className="relative">
            <Search
              size={13}
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-0.5 scrollbar-none">
            {([
              ...
              <button
                key={f}
                onClick={() => setFilter(f)}
                className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border cursor-pointer transition-all select-none"
                style={
                  filter === f
                    ? f === "all"
            ...
            <span className="ml-auto text-xs shrink-0 pl-1" style={{ color: "var(--cf-text3)" }}>
        ...
        {/* Lista de transações */}
        {grouped.length === 0 ? (
          <div className="cf-card p-10 flex flex-col items-center text-center gap-2">
            <div
              className="w-12 h-12 rounded-2xl flex items-center justify-center mb-1"
              style={{ background: "var(--cf-input)" }}
            >
              <TrendingUp size={20} style={{ color: "var(--cf-border)" }} />
            </div>
            <p className="font-bold text-sm" style={{ color: "var(--cf-text)" }}>
              {search || filter !== "all" ? "Nenhum resultado" : "Nenhuma transação ainda"}
            </p>
            <p className="text-xs" style={{ color: "var(--cf-text2)" }}>
              {search || filter !== "all"
                ? "Tente mudar os filtros."
                : "Adicione manualmente ou importe seu extrato bancário."}
            </p>
            {!search && filter === "all" && (
                ...
                <div key={date} className="cf-card overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Calendar size={11} style={{ color: "var(--cf-text3)" }} />
                      <span className="text-xs font-semibold" style={{ color: "var(--cf-text2)" }}>
                        {labelDate(date)}
                      </span>
                      <span
                        className="text-xs px-1.5 py-0.5 rounded-full"
                        style={{ background: "var(--cf-border)", color: "var(--cf-text2)" }}
                      >
                        {items.length}
                      </span>
                    </div>
                  ...
                  {items.map((tx) => {
                    ...
                    return (
                      <div
                        key={tx.id}
                        className="cf-tx flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3"
                      >
                        <div
                        ...
                        >
                          <CatIcon
                            size={16}
                            style={{ color: tx.type === "entrada" ? "#16a34a" : "#dc2626" }}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start sm:items-center gap-1 flex-col sm:flex-row sm:gap-2">
                            ...
                            <span
                              ...
                              style={{
                                background: tx.type === "entrada" ? "#dcfce7" : "#fee2e2",
                                color: tx.type === "entrada" ? "#15803d" : "#b91c1c",
                              }}
                            >
                              {tx.category}
                            </span>
                        ...
                        <div className="cf-txa flex items-center gap-1 shrink-0">
                          <button
                            onClick={() => { setEditing(tx); setModal(true); }}
      ...
      <div className="lg:hidden fixed z-20 flex flex-col gap-2" style={{ bottom: 74, right: 16 }}>
        <button
          ...
          style={{
            background: dark ? "var(--cf-card2)" : "#eff6ff",
            border: "1px solid var(--cf-border)",
            color: "#3b82f6",
          }}
        >

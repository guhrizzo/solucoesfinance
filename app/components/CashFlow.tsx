...
import {
  ...
  Paperclip, ExternalLink, FileScan, Menu, Moon, Sun, HelpCircle,
  UserCircle, Zap,
  type LucideIcon,
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { icon: TrendingUp,      label: "Fluxo de caixa",   href: "/fluxo-caixa",  badge: undefined, active: true },
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="w-full sm:max-w-md bg-white sm:rounded-2xl rounded-t-3xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          ...
          <button onClick={() => !saving && onClose()} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors cursor-pointer"><X size={15} /></button>
        </div>
        <div className="px-5 pt-4 pb-6 space-y-4 overflow-y-auto" style={{ maxHeight: "82vh" }}>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1.5"><Paperclip size={12} /> Nota Fiscal <span className="text-slate-300 font-normal normal-case">(opcional)</span></label>
            {hasNf ? (
              <div className="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-xl">
                <div className="w-9 h-9 rounded-lg bg-blue-100 flex items-center justify-center shrink-0"><FileText size={16} className="text-blue-600" /></div>
                <div className="flex-1 min-w-0"><p className="text-blue-950 text-xs font-semibold truncate">{nfName || "Nota Fiscal"}</p><p className="text-blue-400 text-xs">Anexada ✓</p></div>
                <div className="flex items-center gap-1 shrink-0">
                  {(nfPreview || nfUrl) && (
                    <a href={nfPreview || nfUrl} target="_blank" rel="noreferrer" className="w-7 h-7 rounded-lg bg-blue-100 hover:bg-blue-200 flex items-center justify-center text-blue-600 transition-colors cursor-pointer"><ExternalLink size={12} /></a>
                  )}
                  <button onClick={() => { setNfFile(null); setNfPreview(""); setNfUrl(""); setNfName(""); }} className="w-7 h-7 rounded-lg bg-blue-100 hover:bg-rose-100 hover:text-rose-500 flex items-center justify-center text-blue-400 transition-colors cursor-pointer"><X size={12} /></button>
                </div>
              </div>
            ) : (
              <button onClick={() => nfRef.current?.click()} className="w-full border-2 border-dashed border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 rounded-xl p-4 flex items-center gap-3 transition-all cursor-pointer group">
                <div className="w-9 h-9 rounded-lg bg-slate-100 group-hover:bg-blue-100 flex items-center justify-center shrink-0 transition-colors"><Upload size={15} className="text-slate-400 group-hover:text-blue-500 transition-colors" /></div>
                <div className="text-left"><p className="text-sm font-semibold text-slate-600 group-hover:text-blue-700 transition-colors">Anexar NF</p><p className="text-xs text-slate-400">PDF, imagem ou XML</p></div>
            ...
            {nfFile && (
              <button onClick={scanNf} disabled={scanning}
                className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${scanning ? "border-violet-200 bg-violet-50 text-violet-400 cursor-not-allowed" : "border-violet-300 bg-violet-50 hover:bg-violet-100 text-violet-700 cursor-pointer"}`}>
          ...
          <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-xl">
            {(["entrada", "saida"] as TxType[]).map((t) => (
              <button key={t} onClick={() => { setType(t); setCat(""); }}
                className={`flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${type === t ? t === "entrada" ? "bg-emerald-500 text-white shadow-md" : "bg-rose-500 text-white shadow-md" : "text-slate-400 hover:text-slate-600"}`}>
          ...
          <div className="space-y-1.5">
            ...
            <input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Ex: Cliente XYZ, Aluguel…" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-blue-950 outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Categoria *</label>
            <div className="relative">
              <select value={cat} onChange={(e) => setCat(e.target.value)} className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 pr-9 text-sm text-blue-950 outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all cursor-pointer">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <input inputMode="decimal" value={rawAmt} onChange={(e) => setRawAmt(e.target.value)} placeholder="0,00" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-blue-950 outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all" />
            ...
            <div className="space-y-1.5">
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-blue-950 outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all cursor-pointer" />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wide">Observação <span className="text-slate-300 font-normal normal-case">(opcional)</span></label>
            <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="NF, contrato, parcela…" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-blue-950 outline-none focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100 transition-all" />
          </div>
          <button onClick={submit} disabled={!canSave || saving}
            className={`w-full py-3.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${canSave && !saving ? type === "entrada" ? "bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-100 cursor-pointer" : "bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-100 cursor-pointer" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
      ...
      <div className="w-full sm:max-w-lg bg-white sm:rounded-2xl rounded-t-3xl overflow-hidden"
        ...
        <div className="flex justify-center pt-3 pb-1 sm:hidden shrink-0"><div className="w-10 h-1.5 rounded-full bg-slate-200" /></div>
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-blue-50 flex items-center justify-center"><Sparkles size={15} className="text-blue-600" /></div>
            <div>
          ...
          {step !== "saving" && (
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors cursor-pointer"><X size={15} /></button>
          )}
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {step === "input" && (
            <>
              {errMsg && <div className="bg-rose-50 border border-rose-200 rounded-xl px-3 py-2.5 text-rose-600 text-xs flex items-start gap-2"><span className="shrink-0 mt-0.5">⚠</span> {errMsg}</div>}
              <button onClick={() => fileRef.current?.click()} className="w-full border-2 border-dashed border-slate-200 hover:border-blue-300 hover:bg-blue-50/50 rounded-xl p-5 flex flex-col items-center gap-2 transition-all cursor-pointer group">
                <div className="w-10 h-10 rounded-xl bg-slate-100 group-hover:bg-blue-100 flex items-center justify-center transition-colors"><Upload size={18} className="text-slate-400 group-hover:text-blue-500 transition-colors" /></div>
              ...
              <div className="flex items-center gap-3"><div className="flex-1 h-px bg-slate-100" /><span className="text-xs text-slate-400 font-medium">ou cole o texto</span><div className="flex-1 h-px bg-slate-100" /></div>
              <div className="space-y-1.5">
          ...
          {step === "loading" && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              <div className="relative">
                <div className="w-16 h-16 rounded-2xl bg-blue-50 flex items-center justify-center"><Sparkles size={28} className="text-blue-500" /></div>
                <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-white flex items-center justify-center"><Loader2 size={13} className="text-blue-500 animate-spin" /></div>
              </div>
              ...
              <div className="w-48 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full animate-pulse" style={{ width: "60%" }} /></div>
            </div>
          )}
          {(step === "preview" || step === "saving") && (
            <>
              ...
              <button onClick={toggleAll} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 hover:bg-slate-50 transition-colors cursor-pointer text-sm text-slate-600 font-medium">
                ...
                {preview.map((tx, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => toggle(i)}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 transition-all cursor-pointer text-left ${isSel ? tx.type === "entrada" ? "border-emerald-200 bg-emerald-50/60" : "border-rose-200 bg-rose-50/60" : "border-slate-100 bg-slate-50/60 opacity-50"}`}>
          ...
          {step === "done" && (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              ...
              <p className="text-slate-400 text-sm text-center">{selected.size} transação{selected.size !== 1 ? "ões" : ""} adicionada{selected.size !== 1 ? "s" : ""} ao fluxo de caixa.</p>
            </div>
          )}
        </div>
        <div className="px-5 py-4 border-t border-slate-100 shrink-0 flex gap-2">
          {step === "input" && (
            ...
            <button onClick={analyze} disabled={!text.trim()} className={`flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${text.trim() ? "bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-100 cursor-pointer" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>
          ...
          {step === "preview" && (
            ...
            <button onClick={confirmImport} disabled={selected.size === 0} className={`flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${selected.size > 0 ? "bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-100 cursor-pointer" : "bg-slate-200 text-slate-400 cursor-not-allowed"}`}>
              <Check size={14} /> Importar {selected.size}
            </button></>
          )}
          {step === "saving" && <button disabled className="flex-1 py-3 rounded-xl bg-slate-200 text-slate-400 text-sm font-bold flex items-center justify-center gap-2 cursor-not-allowed"><Loader2 size={14} className="animate-spin" /> Salvando…</button>}
          {step === "done"   && <button onClick={onClose} className="flex-1 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold flex items-center justify-center gap-2 transition-colors cursor-pointer"><Check size={14} /> Concluir</button>}
        </div>
      </div>
    </div>
  );
...
export default function CashFlowPage() {
  ...
  if (pageState === "error") return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center gap-4 p-6 text-center">
      ...
      <button onClick={() => window.location.reload()} className="mt-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors cursor-pointer">Tentar novamente</button>
    </div>
  );

  const userObj = { displayName: userName, email: userEmail };
    ...
    <div className="flex flex-col bg-slate-50 min-h-screen">
      <style>{`
        ...
        *{font-family:'Sora',sans-serif;box-sizing:border-box}
        ...
        /* ── Navbar ── */
        .nxfi-nav{position:sticky;top:0;z-index:50;background:white;border-bottom:1px solid #e8eef8;box-shadow:0 1px 0 #e8eef8,0 4px 20px rgba(13,34,71,.04);display:flex;align-items:center;padding:0 1.5rem;height:64px;gap:1rem}
        .nxfi-logo{display:flex;align-items:center;gap:10px;text-decoration:none;flex-shrink:0}
        .nxfi-logo-icon{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#1565c0,#42a5f5);display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(21,101,192,.3)}
        ...
        .nxfi-nav-link{display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:10px;text-decoration:none;font-size:.8rem;font-weight:500;color:#64748b;white-space:nowrap;transition:all .15s;border:1.5px solid transparent}
        .nxfi-nav-link:hover{color:#1565c0;background:#f0f7ff}
        .nxfi-nav-link.active{color:#1565c0;background:rgba(21,101,192,.08);border-color:rgba(21,101,192,.15);font-weight:600}
        .nxfi-nav-link .badge{background:#ef4444;color:white;font-size:.65rem;font-weight:700;padding:1px 5px;border-radius:100px;font-family:'JetBrains Mono',monospace}
        .nxfi-actions{display:flex;align-items:center;gap:8px;flex-shrink:0}
        .nxfi-search-wrap{position:relative}
        .nxfi-search{background:#f1f5fb;border:1.5px solid transparent;border-radius:10px;padding:7px 12px 7px 34px;font-size:.8rem;color:#0d2247;outline:none;width:200px;transition:all .2s;font-family:'Sora',sans-serif}
        .nxfi-search:focus{border-color:#1565c0;background:white;width:240px;box-shadow:0 0 0 3px rgba(21,101,192,.08)}
        ...
        .nxfi-icon-btn{width:36px;height:36px;border:none;cursor:pointer;border-radius:10px;background:#f8faff;color:#64748b;display:flex;align-items:center;justify-content:center;transition:all .15s;position:relative}
        .nxfi-icon-btn:hover{background:#f0f7ff;color:#1565c0}
        .nxfi-notif-dot{position:absolute;top:7px;right:7px;width:7px;height:7px;border-radius:50%;background:#ef4444;border:1.5px solid white}
        .nxfi-avatar-btn{display:flex;align-items:center;gap:8px;background:none;border:none;cursor:pointer;padding:4px 8px 4px 4px;border-radius:12px;transition:background .15s}
        .nxfi-avatar-btn:hover{background:#f0f7ff}
        .nxfi-avatar{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#1565c0,#42a5f5);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:.875rem;flex-shrink:0}
        .nxfi-avatar-info{text-align:left}
        .nxfi-avatar-name{font-size:.78rem;font-weight:600;color:#0d2247;line-height:1.2}
        .nxfi-avatar-role{font-size:.68rem;color:#94a3b8}
        .nxfi-dropdown{position:absolute;top:calc(100% + 8px);background:white;border:1px solid #e8eef8;border-radius:14px;z-index:100;overflow:hidden;box-shadow:0 8px 32px rgba(13,34,71,.12),0 2px 8px rgba(13,34,71,.06);animation:dropIn .18s cubic-bezier(.22,.68,0,1.1) both}
        ...
        .nxfi-notif-header h3{font-size:.82rem;font-weight:700;color:#0d2247}
        .nxfi-notif-header button{font-size:.72rem;color:#1565c0;background:none;border:none;cursor:pointer;font-weight:600;font-family:'Sora',sans-serif}
        .nxfi-notif-item{display:flex;gap:10px;padding:12px 16px;border-bottom:1px solid #f8faff;transition:background .12s;cursor:pointer}
        ...
        .nxfi-notif-title{font-size:.78rem;font-weight:600;color:#0d2247}
        .nxfi-notif-desc{font-size:.72rem;color:#64748b;margin-top:2px;line-height:1.4}
        .nxfi-notif-time{font-size:.68rem;color:#94a3b8;margin-top:4px;font-family:'JetBrains Mono',monospace}
        .nxfi-user-drop{right:0;width:220px}
        .nxfi-user-drop-header{padding:14px 16px 12px;border-bottom:1px solid #f1f5fb}
        .nxfi-user-drop-name{font-size:.82rem;font-weight:700;color:#0d2247}
        .nxfi-user-drop-email{font-size:.72rem;color:#94a3b8;margin-top:2px}
        .nxfi-drop-item{display:flex;align-items:center;gap:10px;padding:9px 16px;font-size:.8rem;font-weight:500;color:#475569;cursor:pointer;transition:all .12s;background:none;border:none;width:100%;text-align:left;font-family:'Sora',sans-serif}
        ...
        .nxfi-sidebar-overlay{position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:60;backdrop-filter:blur(2px);animation:fadeIn .2s ease both}
        @keyframes fadeIn{from{opacity:0}to{opacity:1}}
        .nxfi-sidebar{position:fixed;top:0;left:0;height:100vh;z-index:70;width:260px;display:flex;flex-direction:column;background:linear-gradient(180deg,#0a1628 0%,#0d2247 60%,#0e3372 100%);border-right:1px solid rgba(255,255,255,.05);animation:slideInLeft .25s cubic-bezier(.22,.68,0,1.1) both;overflow-y:auto}
        @keyframes slideInLeft{from{transform:translateX(-100%)}to{transform:translateX(0)}}
        .nxfi-sidebar-logo{display:flex;align-items:center;justify-content:space-between;padding:20px 20px 18px;border-bottom:1px solid rgba(255,255,255,.06)}
        .nxfi-sidebar-logo-inner{display:flex;align-items:center;gap:10px}
        .nxfi-sidebar-logo-text{font-size:1.1rem;font-weight:800;color:white;letter-spacing:-.02em}
        .nxfi-sidebar-logo-text span{color:#42a5f5}
        .nxfi-sidebar-close{background:rgba(255,255,255,.06);border:none;border-radius:8px;width:30px;height:30px;cursor:pointer;color:rgba(255,255,255,.4);display:flex;align-items:center;justify-content:center;transition:all .15s}
        .nxfi-sidebar-close:hover{background:rgba(255,255,255,.12);color:white}
        .nxfi-sidebar-nav{flex:1;padding:16px 12px}
        .nxfi-sidebar-section{font-size:.65rem;font-weight:700;color:rgba(255,255,255,.25);letter-spacing:.1em;text-transform:uppercase;padding:0 12px;margin:12px 0 6px}
        .nxfi-sidebar-link{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;text-decoration:none;font-size:.82rem;font-weight:500;color:rgba(147,197,253,.55);margin-bottom:2px;transition:all .15s;border:1.5px solid transparent}
        .nxfi-sidebar-link:hover{color:white;background:rgba(255,255,255,.06)}
        .nxfi-sidebar-link.active{color:white;background:rgba(21,101,192,.25);border-color:rgba(66,165,245,.2);font-weight:600}
        .nxfi-sidebar-link .dot{width:6px;height:6px;border-radius:50%;background:#42a5f5;margin-left:auto}
        .nxfi-sidebar-link .badge{margin-left:auto;background:#ef4444;color:white;font-size:.65rem;font-weight:700;padding:1px 6px;border-radius:100px;font-family:'JetBrains Mono',monospace}
        .nxfi-sidebar-footer{padding:12px;border-top:1px solid rgba(255,255,255,.06)}
        .nxfi-sidebar-footer-btn{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;font-size:.82rem;font-weight:500;cursor:pointer;width:100%;background:none;border:none;transition:all .15s;font-family:'Sora',sans-serif;color:rgba(147,197,253,.55)}
        .nxfi-sidebar-footer-btn:hover{color:white;background:rgba(255,255,255,.06)}
        .nxfi-sidebar-footer-btn.danger:hover{color:#fca5a5;background:rgba(239,68,68,.08)}
        .nxfi-mobile-nav{position:fixed;bottom:0;left:0;right:0;z-index:40;background:white;border-top:1px solid #e8eef8;box-shadow:0 -4px 20px rgba(13,34,71,.06);display:flex;justify-content:space-around;align-items:center;padding:6px 8px 10px}
        .nxfi-mobile-link{display:flex;flex-direction:column;align-items:center;gap:3px;padding:6px 12px;border-radius:12px;text-decoration:none;transition:all .15s;color:#94a3b8;font-size:.65rem;font-weight:500}
        .nxfi-mobile-link:hover{color:#1565c0;background:#f0f7ff}
        .nxfi-mobile-link.active{color:#1565c0}
        @media(max-width:1024px){.nxfi-nav-links,.nxfi-search-wrap{display:none}.nxfi-avatar-info{display:none}}
      ...
      {confirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(10,22,40,0.65)", backdropFilter: "blur(4px)" }}>
          <div className="card p-6 w-full max-w-xs text-center">
            <div className="w-11 h-11 rounded-full bg-rose-50 flex items-center justify-center mx-auto mb-3"><Trash2 size={18} className="text-rose-500" /></div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setConfirmId(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-500 text-sm font-semibold hover:bg-slate-50 transition-colors cursor-pointer">Cancelar</button>
              <button onClick={handleDelete} disabled={deleting} className="flex-1 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-sm font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed">
      ...
      <main className="flex-1 p-3 sm:p-4 md:p-6 space-y-3 sm:space-y-4 overflow-y-auto overflow-x-hidden pb-24 lg:pb-6">
        ...
        <div className="flex items-center justify-between gap-2">
          <div>
            <h1 className="text-blue-950 font-bold text-lg leading-tight">Fluxo de caixa</h1>
            <p className="text-slate-400 text-xs mt-0.5">Olá, {userName} 👋</p>
          </div>
          <div className="hidden sm:flex items-center gap-2">
            <button onClick={() => setImportOpen(true)}
              className="flex items-center gap-1.5 border border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold px-3 py-2 rounded-xl transition-colors cursor-pointer">
              <Sparkles size={13} /> Importar extrato
            </button>
            <button onClick={() => { setEditing(null); setModal(true); }}
              className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition-colors cursor-pointer">
        ...
        <button onClick={() => setImportOpen(true)} className="sm:hidden w-full card p-3 flex items-center gap-3 cursor-pointer hover:border-blue-200 transition-colors">
          <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center shrink-0"><Sparkles size={16} className="text-blue-600" /></div>
            ...
            {([
              ...
              <button key={f} onClick={() => setFilter(f)}
                className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border-2 cursor-pointer transition-all select-none
                  ${filter === f ? f === "all" ? "bg-slate-800 text-white border-transparent" : f === "entrada" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200" : "bg-white text-slate-400 border-slate-200 hover:border-slate-300"}`}>
                {Icon && <Icon size={13} />}
                {label}
              </button>
            ))}
            <span className="ml-auto text-xs text-slate-400 shrink-0 pl-1">{filtered.length} resultado{filtered.length !== 1 ? "s" : ""}</span>
          </div>
        </div>
        ...
        {grouped.length === 0 ? (
          <div className="card p-8 sm:p-10 flex flex-col items-center text-center gap-2">
            <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center mb-1"><TrendingUp size={22} className="text-blue-200" /></div>
            <p className="text-blue-950 font-bold text-sm">{search || filter !== "all" ? "Nenhum resultado" : "Nenhuma transação ainda"}</p>
            <p className="text-slate-400 text-xs">{search || filter !== "all" ? "Tente mudar os filtros." : "Adicione manualmente ou importe seu extrato bancário."}</p>
            {!search && filter === "all" && (
              <div className="flex gap-2 mt-2">
                <button onClick={() => { setEditing(null); setModal(true); }} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors cursor-pointer"><Plus size={14} /> Adicionar</button>
                <button onClick={() => setImportOpen(true)} className="flex items-center gap-2 border border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-bold px-4 py-2.5 rounded-xl transition-colors cursor-pointer"><Sparkles size={14} /> Importar extrato</button>
              </div>
            )}
          </div>
        ) : (
              ...
              return (
                <div key={date} className="card overflow-hidden">
                  <div className="flex items-center justify-between px-3 sm:px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                    ...
                    <span className={`mono text-xs font-bold ${net >= 0 ? "text-emerald-600" : "text-rose-500"}`}>{net >= 0 ? "+" : ""}{toBRL(net)}</span>
                  </div>
                  {items.map((tx) => {
                    const CatIcon = CAT_ICON[tx.category] ?? (tx.type === "entrada" ? ArrowUpRight : ArrowDownRight);
                    return (
                      <div key={tx.id} className="tx flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start sm:items-center gap-1 flex-col sm:flex-row sm:gap-2">
                            ...
                            <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full shrink-0 ${tx.type === "entrada" ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{tx.category}</span>
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            {tx.note && <p className="text-slate-400 text-xs truncate">{tx.note}</p>}
                            {tx.nfUrl && (
                              <a href={tx.nfUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 font-medium shrink-0 transition-colors" title={tx.nfName ?? "Ver Nota Fiscal"}>
                                <Paperclip size={11} />
                        ...
                        <span className={`mono text-sm font-bold shrink-0 ${tx.type === "entrada" ? "text-emerald-600" : "text-rose-500"}`}>{tx.type === "entrada" ? "+" : "-"}{toBRL(tx.amount)}</span>
                        <div className="txa flex items-center gap-1 shrink-0">
                          <button onClick={() => { setEditing(tx); setModal(true); }} className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-blue-50 hover:text-blue-600 flex items-center justify-center text-slate-400 transition-colors cursor-pointer"><Edit3 size={12} /></button>
                          <button onClick={() => setConfirmId(tx.id)} className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-rose-50 hover:text-rose-500 flex items-center justify-center text-slate-400 transition-colors cursor-pointer"><Trash2 size={12} /></button>
                        </div>
                      </div>
                    );
                  })}
      ...
      <div className="lg:hidden fixed z-20 flex flex-col gap-2" style={{ bottom: 74, right: 18 }}>
        <button onClick={() => setImportOpen(true)} className="w-12 h-12 rounded-xl bg-blue-50 border border-blue-200 text-blue-600 flex items-center justify-center active:scale-95 transition-transform cursor-pointer" style={{ boxShadow: "0 4px 16px rgba(21,101,192,.2)" }}>
          <Sparkles size={18} />
        </button>
        <button onClick={() => { setEditing(null); setModal(true); }} className="bg-blue-600 text-white rounded-2xl flex items-center justify-center active:scale-95 transition-transform cursor-pointer" style={{ width: 54, height: 54, boxShadow: "0 8px 28px rgba(21,101,192,.5)" }}>
          <Plus size={24} />
        </button>
      </div>
    </div>
